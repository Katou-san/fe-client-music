...
export const Artist = {
  Get_Current: async (): Promise<any> =>
    await http.get(
      `${EnvConfig.NEXT_PUBLIC_CLIENT}${EnvConfig.NEXT_PUBLIC_ARTIST}`,
      header()
    ),
  Get_Type: async (type: boolean): Promise<any> =>
    await http.get(
      `${EnvConfig.NEXT_PUBLIC_CLIENT}${EnvConfig.NEXT_PUBLIC_ARTISTS}/${type}`,
      header()
    ),
  Get_All: async (): Promise<any> =>
    await http.get(
      `${EnvConfig.NEXT_PUBLIC_CLIENT}${EnvConfig.NEXT_PUBLIC_ARTISTS}/all`,
      header()
    ),
  Search: async (value: string): Promise<any> =>
    await http.get(
      `${EnvConfig.NEXT_PUBLIC_CLIENT}${EnvConfig.NEXT_PUBLIC_ARTIST_SEARCH}/${value}`,
      header()
    ),
  ...
  Update: async (id: string, body: update_artistType): Promise<any> =>
    await http.put(
      `${EnvConfig.NEXT_PUBLIC_CLIENT}${EnvConfig.NEXT_PUBLIC_ARTIST}/${id}`,
      body,
      header()
    ),
