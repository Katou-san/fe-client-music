...
export default function MiniItem({ Value_Form, list, index, active, imageFile, audioFile }: Props) {
  const [urlImg, set_UrlImg] = useState('')
  const [audioSrc] = useState(new Audio(audioFile ? URL.createObjectURL(audioFile) : ''))
  const [Togo_Play, Set_Togo_Play] = useState(false);

  useEffect(() => {
    if (Value_Form.Song_Image != '' && Value_Form.Song_Image != null) {
      set_UrlImg(URL.createObjectURL(imageFile))
    }
